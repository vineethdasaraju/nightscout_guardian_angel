...
public class graphUpdateAsyncTask extends AsyncTask<Void, Void, Void> {
    ...
    public graphUpdateAsyncTask(Context context, JSONObject obj, Activity app) {
    ...
    protected void onPostExecute(Void result) {
        ...
        pd.dismiss();
        if (success) {
            Functions.toast("Graph has been updated.", context);
            app.finish();
        } else {
            Functions.toast("Could not update the graph.", context);
        }
    }
